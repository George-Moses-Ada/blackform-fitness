...
// Mock data (will be populated from frontend data)
const trainers = [
  {
    name: 'Sarah Johnson',
...
async function seed() {
  try {
    await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/blackform-fitness');
    ...
    for (let i = 0; i < createdTrainers.length; i++) {
      const trainerPrograms = createdPrograms.filter(p => p.trainer.equals(createdTrainers[i]._id));
      createdTrainers[i].programs = trainerPrograms.map(p => p._id);
      await createdTrainers[i].save();
    }
